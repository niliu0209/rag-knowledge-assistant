...
# 检索相关阈值（S1-1 初校 0.98 → S1-4 重校准 1.025，2026-08-11）：
# Chroma 集合为默认 l2 空间；真实文档（bge-m3 中文）实测距离分布——
# 标准问法命中 top-1 ≤0.927，但问法变体"8月全品类盘点是什么时候？"对同一
# 命中片段 top-1 为 1.0179（S1-1 的 0.98 将其误过滤，用户实测确认应命中）；
# 无关问法 top-1 ≥1.031（同问法跨时点测量漂移 ~0.0115）。间隔 [1.0179, 1.0310]
# 仅 0.0125 且与测量漂移同量级——固定阈值已达信息极限，1.025 为唯一均衡点
# （救回变体命中、无关全滤；0.98~1.025 之间的低相关片段进入引用可核对，
# prompt 诚实规则兜底）。distance > 阈值的片段视为无关，不进入引用——
# 检索层判定替代"依赖 LLM 判断片段无关"（阶段 0 remaining_risk）。
RELEVANCE_DISTANCE_THRESHOLD: float = 1.025
